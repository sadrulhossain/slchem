...
class LedgerReportController extends Controller {
    ...
    public function index(Request $request) {
        ...
        if ($request->generate == 'true') {
            ...
            $currentTime = !empty($request->from_date) ? $request->from_date . ' 00:00:00' : '';
            $lastTime = !empty($request->to_date) ? $request->to_date . ' 23:59:59' : '';
            ...
            //query for displaying current Adjustment and Production data
            $crConsumpAdjArr = LotWiseConsumptionDetails::join('pro_consumption_details', 'pro_consumption_details.id', '=', 'pro_consumption_lot_wise_details.consump_details_id')
                    ->join('pro_consumption_master', 'pro_consumption_master.id', '=', 'pro_consumption_details.master_id')
                    ->select('pro_consumption_master.adjustment_date'
                            , 'pro_consumption_master.source'
                            , 'pro_consumption_lot_wise_details.id'
                            , 'pro_consumption_lot_wise_details.rate'
                            , 'pro_consumption_lot_wise_details.quantity'
                            , 'pro_consumption_lot_wise_details.amount'
...
//                                    )
            ...
            //query for displaying current Substore data
            $crSubstoreArr = LotWiseConsumptionDetails::join('pro_consumption_details', 'pro_consumption_details.id', '=', 'pro_consumption_lot_wise_details.consump_details_id')
                    ->join('pro_consumption_master', 'pro_consumption_master.id', '=', 'pro_consumption_details.master_id')
                    ->select('pro_consumption_master.delivered_at'
                            , 'pro_consumption_lot_wise_details.id'
                            , 'pro_consumption_lot_wise_details.rate'
                            , 'pro_consumption_lot_wise_details.quantity'
                            , 'pro_consumption_lot_wise_details.amount'
                            , 'pro_consumption_lot_wise_details.lot_number')
                    ->where('pro_consumption_details.product_id', $request->product_id)
                    ->whereBetween('pro_consumption_master.delivered_at', [$request->from_date . ' 00:00:00', $request->to_date . ' 23:59:59'])
                    ->where('pro_consumption_master.status', 1)
...
////                                    , 'pro_consumption_lot_wise_details.id'
//                                    )
                    ->orderBy('pro_consumption_master.delivered_at', 'asc')
                    ->orderBy('pro_consumption_lot_wise_details.id', 'asc')
                    ->get();

            // echo '<pre>';
//            print_r($crConsumpAdjArr);
            //print_r($crSubstoreArr);
            //exit;
            //prepare data for displaying checkin side
            $crConsumpArr = [];
            if (!$crConsumpAdjArr->isEmpty()) {
                foreach ($crConsumpAdjArr as $conAdj) {
                    $adjustmentDate = $conAdj->adjustment_date;
                    $crConsumpArr[$adjustmentDate][$conAdj->lot_number]['quantity'] = !empty($crConsumpArr[$adjustmentDate][$conAdj->lot_number]['quantity']) ? $crConsumpArr[$adjustmentDate][$conAdj->lot_number]['quantity'] : 0;
                    $crConsumpArr[$adjustmentDate][$conAdj->lot_number]['quantity'] += $conAdj->quantity;
                    $crConsumpArr[$adjustmentDate][$conAdj->lot_number]['amount'] = !empty($crConsumpArr[$adjustmentDate][$conAdj->lot_number]['amount']) ? $crConsumpArr[$adjustmentDate][$conAdj->lot_number]['amount'] : 0;
                    $crConsumpArr[$adjustmentDate][$conAdj->lot_number]['amount'] += $conAdj->amount;
                    $crConsumpArr[$adjustmentDate][$conAdj->lot_number]['rate'] = $conAdj->rate;
                    $crConsumpArr[$adjustmentDate][$conAdj->lot_number]['lot_number'] = $conAdj->lot_number;
                }
            }
//            echo '<pre>';
//            print_r($crConsumpArr);
//            exit;

            if (!$crSubstoreArr->isEmpty()) {
                foreach ($crSubstoreArr as $sub) {
                    $deliverdAt = date('Y-m-d', strtotime($sub->delivered_at));
                    $crConsumpArr[$deliverdAt][$sub->lot_number]['quantity'] = !empty($crConsumpArr[$deliverdAt][$sub->lot_number]['quantity']) ? $crConsumpArr[$deliverdAt][$sub->lot_number]['quantity'] : 0;
                    $crConsumpArr[$deliverdAt][$sub->lot_number]['quantity'] += $sub->quantity;
                    $crConsumpArr[$deliverdAt][$sub->lot_number]['amount'] = !empty($crConsumpArr[$deliverdAt][$sub->lot_number]['amount']) ? $crConsumpArr[$deliverdAt][$sub->lot_number]['amount'] : 0;
                    $crConsumpArr[$deliverdAt][$sub->lot_number]['amount'] += $sub->amount;
                    $crConsumpArr[$deliverdAt][$sub->lot_number]['rate'] = $sub->rate;
                    $crConsumpArr[$deliverdAt][$sub->lot_number]['lot_number'] = $sub->lot_number;
                }
            }
...
//            $crCheckinInfo = $daywiseCheckIn = [];
            ...
            if (!empty($crConsumpArr)) {
                foreach ($crConsumpArr as $date => $itemDetails) {
                    foreach ($itemDetails as $lotNumber => $item) {
                        if ($testadjustmentDate != $date) {
                            $testadjustmentDate = $date;
                            $j = 0;
                        }
                        $crConsumpInfo[$date][$j]['lot_number'] = $item['lot_number'];
                        $crConsumpInfo[$date][$j]['quantity'] = $item['quantity'];
                        $crConsumpInfo[$date][$j]['rate'] = $item['rate'];
                        $crConsumpInfo[$date][$j]['amount'] = $item['amount'];
                        $j++;
                    }
                }
            }
...
//            exit;
    ...
    public function filter(Request $request) {
        ...
        if ($validator->fails()) {
            return redirect('ledgerReport?product_id=' . $request->product_id . '&from_date=' . $request->from_date . '&to_date=' . $request->to_date)
                            ->withInput()
