...
class InitialBalanceController extends Controller {
    ...
    public function create() {
        $productArr = ['0' => __('label.SELECT_PRODUCT_OPT')] + Product::orderBy('name', 'asc')->where('status', '1')->where('approval_status', 1)->where('initiated', '0')->pluck('name', 'id')->toArray();
        ...
        $balanceSetTime = date('H:i:s');

        if (strtotime($balanceSetTime) <= strtotime($setCutOffTime->check_in_time)) {
        ...
        return view('productCheckIn.setBalance')->with(compact('productArr', 'supplierArr', 'manufacturerArr', 'referenceNo'
                                , 'purposeArr', 'addressArr','checkinDate','balanceSetTime'));
    }
    ...
    public function setBalance(Request $request) {
        ...
        if (!empty($request->add_btn)) {
            if ($target->save()) {
                ...
                //ProductCheckInDetails::where('master_id', $target->id)->delete();
                /* insert data into product check in details table */
                ProductCheckInDetails::insert($data);
                $productDetails = ProductCheckInDetails::where('master_id', $target->id)->get();
