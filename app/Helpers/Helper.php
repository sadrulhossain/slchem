...
//function for getOrderList
    ...
    public static function commaSperateFormat($num = '', $digit = '') {
        return number_format($num, $digit, '.', '');
    }
    ...
    public static function consumeQuantity($consumpMasterId = null, $consumptionDetailId = null, $productId = null, $quantity = null) {
        $lotInfo = ProductCheckInDetails::where('product_id', $productId)->where('consumed', '0')
                        ->orderBy('id', 'asc')->lockForUpdate()->first();

        if (!empty($lotInfo)) {
            //echo $remainingQuantity;exit;
            $remainingQty = $quantity - $lotInfo['remaining_quantity'];
            $updateArr['remaining_quantity'] = ($quantity >= $lotInfo['remaining_quantity']) ? '0' : ($lotInfo['remaining_quantity'] - $quantity);
            if ($remainingQty >= 0) {
            ...
            $dataDetails->consump_master_id = $consumpMasterId;
            $dataDetails->consump_details_id = $consumptionDetailId;
            $dataDetails->checkin_details_id = $lotInfo['id'];
            ...
            if ($remainingQty > 0) {
                self::consumeQuantity($consumpMasterId, $consumptionDetailId, $productId, $remainingQty);
            }
            return true;
        } else {
    ...
    public static function unitConversion($totalQtyStr = "") {
        ...
        if ($kgAmnt > 0) {
//            $kgAmnt = (int) $kgAmnt;
            $kgFinalAmntStr = $kgAmnt . " " . __('label.UNIT_KG');
        }
        ...
        if ($pos !== false) { //If decimal point exists
            ...
            $gmFinalAmntStr = "";
            if ($gmStr > 0) {
                $gmFinalAmntStr = $gmStr . " " . __('label.GM');
            }
            $miliGmStr = substr($totalAmntStr, 3, 3); //Subtract miligram aamount
            $mgFinalAmntStr = "";
            if ($miliGmStr > 0) {
                $mgFinalAmntStr = $miliGmStr . " " . __('label.MG');
            }
