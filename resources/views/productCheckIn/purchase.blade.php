...
<script type="text/javascript">
    ...
    $(document).ready(function() {
        $(document).on("change", '#productId', function() {
            ...
            $.ajax({
                url: "{{URL::to('productCheckIn/getSupplierManufacturer')}}",
                type: 'POST',
                dataType: 'json',
                headers: {
        ...
        $(document).on("change", '#manufacturerId', function() {
            ...
            $.ajax({
                url: "{{URL::to('productCheckIn/getManufacturerAddress')}}",
                type: 'POST',
                dataType: 'json',
                headers: {
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
        ...
        $('#addItem').click(function() {
            ...
            $.ajax({
                url: "{{ URL::to('productCheckIn/purchaseNew')}}",
                type: "POST",
                dataType: 'json',
                cache: false,
                headers: {
...
//                $("#factoryLabel").prop("checked", false);
                ...
                <td>\n\<input type="hidden" name="add_btn" value="1">\n\
                    ...
                    <input type="hidden" id="quantity_' + productId + '_' + countNumber + '"  name="quantity[]"  value="' + parseFloat(quantity1).toFixed(6) + '">\n\
                ...
                <td>' + lotNumber + '</td>\n\
                <td class="text-center">' + parseFloat(quantity1).toFixed(6) + ' ' + result.productUnit + '</td>\n\
        ...
        $('#productId').on('change', function() {
            ...
            if (productId != '0') {
                $.ajax({
                    url: "{{URL::to('productCheckIn/productHints')}}",
                    type: "POST",
        ...
        $(document).on("click", ".button-submit", function(e) {
            ...
            swal({
                ...
                if (isConfirm)
                    $.ajax({
                        url: "{{URL::to('/productCheckIn/purchaseProduct')}}",
                        type: "POST",
